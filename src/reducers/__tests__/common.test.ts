...
import { ARTICLE, ERRORS, SETTINGS_PAYLOAD } from "../../mock";
import commonReducer from "../common";

describe("common reducer", () => {
  it("passes app lifecycle", () => {
    const initialState = commonReducer(undefined, {});
    const afterLoadStateLogin = commonReducer(initialState, {
      type: APP_LOAD,
    ...
    const afterLoadState = commonReducer(initialState, {
      type: APP_LOAD,
      payload: null,
      token: null,
      skipTracking: true,
    ...
    const afterLogoutState = commonReducer(afterLoadStateLogin, {
      type: LOGOUT,
    });
    ...
    const afterArticleSubmitted = commonReducer(afterLoadStateLogin, {
    ...
    [LOGIN, REGISTER].forEach((actionType) => {
      ...
      expect(
        commonReducer(afterLoadStateLogin, {
          type: actionType,
          error: true,
          payload: { errors: ERRORS },
        })
    ...
    [
      ...
      expect(commonReducer(afterLoadStateLogin, { type: actionType })).toEqual({
        appName: "Conduit",
        viewChangeCounter: 1,
        appLoaded: true,
        token: SETTINGS_PAYLOAD.user.token,
